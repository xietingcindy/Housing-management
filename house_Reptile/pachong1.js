...
let total = 0; // 数据总数
const citys = ['0', '1', '2']; //城市数组
const urls = ['https://bj.lianjia.com/zufang/#contentList', 'https://sh.lianjia.com/zufang/#contentList', 'https://sz.lianjia.com/zufang/#contentList']; // 爬取URL
    ...
    https.get(url, res => {
        ...
        res.on('end', function() {
            ...
            if (page <= endPage) { // 继续抓取下一页
                ...
                let tempUrl = 'https://sz.lianjia.com/zufang/pg' + page + '#contentList/';
...
function filter(data) {
    ...
    items.each((index, item) => {
        ...
        let city = citys[2]; //爬取数据前时可定义
        let titles = $(item).find('.twoline').text().replace(/\s/g, '').split('·');
        let flag = titles[0];
        let title = titles[1];
        // let price = $(item).find('span.content__list--item-price').text(); // 价格
        ...
        let pic = $(item).find('.content__list--item--aside').children('img').attr('src'); //图片
